...
**Supabase project:** `hakiksjnpipgstomzzjy`
...
- The five component factors and their general qualitative descriptions (e.g., "transportation noise levels in your area") — never the weights
- Letter grades and what they mean (A ≥ 80, B 65–79, C 50–64, D 35–49, F < 35)
- All federal data sources: CDC PLACES, BTS, NLCD, USDA, NASA VIIRS, EPA AQS, EPA EJScreen
...
- The composite multi-tool scoring logic
- The `score_config` table contents
- Min/max calibration values used in normalization
...
- **Claude interpretation layer** — plain-language neighborhood health narrative is a meaningful differentiator, hard to replicate at scale
...
- Plain-language interpretations generated by Claude API
- That scores are dated and will be refreshed annually (longitudinal tracking)

### Proprietary (never expose in any file, UI, log, or output)
- Exact component weights
- Normalization formula details and min/max calibration values
- Algorithmic decisions: null handling, outlier treatment, cross-metro calibration
- Claude API interpretation layer: prompt templates, model configuration
- Cross-tool composite methodology (how `overall_scores` are derived)
- The `score_config` table contents

If you are generating any public-facing file (README, Streamlit UI text, Lovable components, website copy), treat the methodology as a black box. Describe *what* is measured, never *how* it is calculated.
...
**Scope:** Validates all pipeline outputs and data integrity across all 6 Supabase score tables.

**Primary responsibilities:**
- Runs `notebooks/qa/qa_data_integrity.py` in Colab — confirms 106/106 tests pass
...
**Primary responsibilities:**
- Tests all 5 tool tabs + overall summary card
- Test ZIPs: 15213 (Pittsburgh), 90210 (Los Angeles), 28277 (Charlotte), 85257 (Phoenix)
- Validates disc visualization, component breakdown, interpretation text, metro comparison
- Checks graceful error handling (invalid ZIP like 00000, empty input)
- References the UI checklist in `notebooks/qa/qa_data_integrity.py` Section 5
...
- Executes all 5 tool pipelines (Respiratory, Cardiovascular, Stress, Food Access, Heat) for new ZIPs
- Runs `overall_pipeline.py` for new ZIPs
- Runs QA suite to verify new metro data passes all 106 tests
- Updates `CONTEXT.md` with new metro coverage numbers

**Critical note:** Global normalization may need recalibration when new metros are added — adding metros with extreme values could shift the min/max anchors and change existing scores. Document any recalibration decisions.

**When to run:** When expanding to new metros (next priority after Lovable migration).

### Agent 4 — Frontend Build (Lovable) — NEXT PRIORITY
**Scope:** Moves the frontend from Streamlit to Lovable at `lasalletech.ai`, consuming Supabase REST API directly.

**Primary responsibilities:**
- Supabase project ref: `hakiksjnpipgstomzzjy` — auto-exposes REST endpoints per table
- Lovable fetches directly using the Supabase anon key — no separate API layer or backend proxy needed
- Rebuilds the disc SVG visualization in React/JavaScript (currently Python/matplotlib in Streamlit)
- Maintains exact same component configs, colors, and grade scales as current `app.py`
- References `TOOL_SPECS.md` for component weights, colors, and data sources
- Maintains the public/proprietary boundary — no weights, normalization details, or methodology in frontend code

**Architecture:** Lovable → Supabase REST API (anon key) → score tables. No middleware. No server-side rendering of scores. All data is pre-computed and stored; frontend is purely a read layer.

**When to run:** Next session priority — this is the current active workstream.
