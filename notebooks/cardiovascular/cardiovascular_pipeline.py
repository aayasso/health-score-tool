...
if not noise_already_done:
    ...
    df_noise = df_noise.rename(columns={"signal_value": "noise_raw"})
...
if not impervious_already_done:
    ...
    for _, row in df_impervious.dropna(subset=["impervious_raw"]).iterrows():
        record = {
            ...
            "data_vintage": 2021,
            "signal_value": float(row["impervious_raw"]),
            "units": "percent",
        }
        try:
            upsert_with_retry(supabase, "raw_signals", record,
    ...
    df_impervious = df_impervious.rename(columns={"signal_value": "impervious_raw"})
...
for col in ["noise_raw", "impervious_raw"]:
    nulls = df[col].isna()
    if nulls.any():
...
for _, row in df.iterrows():
    ...
    try:
        upsert_with_retry(supabase, "cardiovascular_scores", record)
    except Exception as e:
