...
if not viirs_already_done:
    ...
    for _, row in df_viirs.dropna(subset=["light_pollution_raw"]).iterrows():
        record = {
            ...
            "data_vintage": 2024,
            "signal_value": float(row["light_pollution_raw"]),
            "units": "nW/cm2/sr",
        }
        try:
    ...
    df_viirs = df_viirs.rename(columns={"signal_value": "light_pollution_raw"})
...
for _, row in df.iterrows():
    ...
    try:
        upsert_with_retry(supabase, "stress_scores", record)
    except Exception as e:
